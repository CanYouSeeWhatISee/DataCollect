...
public class JedisUtil {
     ...
     *            端口
     */
    public JedisUtil(String ip, int port, int dataBaseIndex, String password) {
        this.dataBaseIndex = dataBaseIndex;
        if (pool == null) {
            ...
            if (password.equals("")) {
                pool = new JedisPool(config, ip, port, 100000, null, dataBaseIndex);
            } else {
                pool = new JedisPool(config, ip, port, 100000, password, dataBaseIndex);
            }
    ...
    public String get(String key) {
        ...
        try {
            ...
            log.error("JedisUtil Exception", e);
        } finally {
            closeJedis(jedis);
        }
        return value;
     ...
     * @return 成功 返回OK 失败返回 0
     */
    public String set(String key, String value, Integer index, int seconds) {
        Jedis jedis = null;
        try {
            jedis = pool.getResource();
            jedis.select(index);
     ...
     * @return 返回OK 异常返回null
     */
    public String hmset(String key, Map<String, String> hash, int index) {
        Jedis jedis = null;
        String res = null;
        try {
            jedis = pool.getResource();
    ...
    /**
     * <p>
     * 提供一个查其他index下的数据的hget接口 通过key 和 field 获取指定的 value 查完之后会select回默认的index
     * </p>
     *
     * @param key
     * @param field
    ...
    public Long lpush(int index, String key, String... strs) {
        Jedis jedis = null;
        Long res = null;
        try {
            jedis = pool.getResource();
    ...
    public Long hset(byte[] bytes, byte[] bytes2, byte[] byteArray) {
        Jedis jedis = null;
        Long res = null;
        try {
            jedis = pool.getResource();
            res = jedis.hset(bytes, bytes2, byteArray);
        } catch (JedisException e) {
            log.error("JedisUtil Exception", e);
        } finally {
            closeJedis(jedis);
    ...
    private void closeJedis(Jedis jedis) {
        if (jedis != null) {
            ...
            if (pool.getNumActive() > 0) {
                log.debug("JedisUtil poolinfo, NumActive=" + pool.getNumActive() + ",NumIdle=" + pool.getNumIdle()
                        + ",NumWaiters=" + pool.getNumWaiters() + ",MaxBorrowWaitTimeMillis="
                        + pool.getMaxBorrowWaitTimeMillis() + ",MeanBorrowWaitTimeMillis="
